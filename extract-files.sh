...
HELPER="$LOTUS_ROOT"/vendor/lotus/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
setup_vendor "$DEVICE_COMMON" "$VENDOR" "$LOTUS_ROOT" true $CLEAN_VENDOR
...
if [ -s "$MY_DIR"/../$DEVICE_SPECIFIED_COMMON/proprietary-files.txt ];then
    ...
    setup_vendor "$DEVICE_SPECIFIED_COMMON" "$VENDOR" "$LOTUS_ROOT" false "$CLEAN_VENDOR"
...
if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
    ...
    setup_vendor "$DEVICE" "$VENDOR" "$LOTUS_ROOT" false "$CLEAN_VENDOR"
